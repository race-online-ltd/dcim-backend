...
class ReportController extends Controller
{
    ...
    public function getSensorLocations(Request $request)
    {
        // Validate inputs coming from frontend
        $validated = $request->validate([
            'data_center_id' => 'required|integer|min:1',
            'sensor_type_list_id' => 'required|integer|min:1',
        ]);

        try {
            $locations = DB::table('sensor_lists as s')
                ->select('s.location')
                ->where('s.data_center_id', $validated['data_center_id'])
                ->where('s.sensor_type_list_id', $validated['sensor_type_list_id'])
                ->where('s.status', 1)
                ->get();
