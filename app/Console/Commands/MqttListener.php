                                    ...
                                    if (is_array($sensorArray)) {
                                        foreach ($sensorArray as $sensor) {
                                            if (isset($sensor['id']) && isset($sensor['val'])) {
                                                ...
                                                // ─── Main Sensor Insert (Transaction 1) ──────────
                                                try {
                                                    DB::beginTransaction();
                                                ...
                                                // ─── Replica Sensor Insert (Transaction 2) ───────
                                                if (isset($replicationMap[(int)$sensor['id']])) {
                                                    $replicaSensorId     = $replicationMap[(int)$sensor['id']]['sensor_id'];
                                                    $replicaDataCenterId = $replicationMap[(int)$sensor['id']]['data_center_id'];
                                                    ...
                                                    try {
                                                        ...
                                                        DB::rollBack();
                                                        Log::error("Failed to replicate sensor data for Replica Sensor ID {$replicaSensorId}: " . $e->getMessage());
                                                        echo "Error replicating sensor data: " . $e->getMessage() . "\n";
                                                    }
                                                }
                                                // ─────────────────────────────────────────────────
                                            }
